...
export class ExportarExcelComponent implements OnInit {
  ...
  habilitado() {
    if (this.localidades.length > 0) {
      ...
      console.log('this.checkArraySexo', this.checkArraySexo)
      ...
      console.log('temp', this.Data.length)
      if (this.checkArray.length > 0) temp[1] = true
      if (temp[0] === true && temp[1] === true) { this.disabled = false }
  ...
  async submitForm() {
    ...
    if (this.localidades.length !== undefined) {
      for (let loc = 0; loc < this.checkArray.value.length; loc++) {
        ...
        let buscarLocalidad = this.localidades.filter(x => x.localidad == localidad)
        this.DataSexo.map((sexo: any) => {
          if (sexo.checked) {
            let buscarSexoLocalidad = buscarLocalidad[0].empadronados.filter(x => x.genero == sexo.value)
      ...
      data.push({ seccion: '' })
      data.push({ seccion: `Total de empadronados: ${this.localidades.total}` })
